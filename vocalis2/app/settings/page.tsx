...
import {getSettings,saveSettings,clearAllSessions,applyTheme} from '@/lib/store'
import type {UserSettings} from '@/lib/types'

export default function SettingsPage(){
  const router=useRouter()
  const [s,setS]=useState<UserSettings|null>(null),[saved,setSaved]=useState(false),[del,setDel]=useState(false)
  useEffect(()=>setS(getSettings()),[])
  const up=(p:Partial<UserSettings>)=>{if(!s)return;setS({...s,...p});setSaved(false)}

  const handleTheme=(theme:'dark'|'light')=>{
    up({theme});applyTheme(theme)
  }

  const handleSave=()=>{
    if(!s)return;saveSettings(s);setSaved(true);setTimeout(()=>setSaved(false),2000)
  }

  const handleDelete=()=>{
    if(!del){setDel(true);return}
    clearAllSessions();router.push('/dashboard')
  }
  ...
  return(<>
    ...
    <div className="container">
      ...
      <h1 className="font-display anim-slide-up anim-d2" style={{fontSize:'clamp(32px,4vw,48px)',fontWeight:900,letterSpacing:'-.04em',marginBottom:'40px'}}>Your Preferences</h1>

      {/* Appearance */}
      <div className="anim-slide-up anim-d3" style={{marginBottom:'24px'}}>
        <p style={{fontSize:'13px',fontWeight:700,letterSpacing:'.08em',color:'var(--text-muted)',marginBottom:'6px'}}>APPEARANCE</p>
        <div className="settings-section">
          <div className="settings-row">
            <div><div style={{fontWeight:600}}>Theme</div><div className="text-muted" style={{fontSize:'13px'}}>Choose light or dark mode</div></div>
      ...
      {/* Profile */}
      <div className="anim-slide-up anim-d3" style={{marginBottom:'24px'}}>
        <p style={{fontSize:'13px',fontWeight:700,letterSpacing:'.08em',color:'var(--text-muted)',marginBottom:'6px'}}>PROFILE</p>
        <div className="settings-section">
          <div className="settings-row">
            <div><div style={{fontWeight:600}}>First Name</div><div className="text-muted" style={{fontSize:'13px'}}>Shown in your dashboard greeting</div></div>
            <input className="input" type="text" value={s.name} onChange={e=>up({name:e.target.value})} style={{width:'180px'}} placeholder="Your name"/>
          </div>
          <div className="settings-row">
            <div><div style={{fontWeight:600}}>Email</div></div>
      ...
      {/* Training */}
      <div className="anim-slide-up anim-d3" style={{marginBottom:'24px'}}>
        <p style={{fontSize:'13px',fontWeight:700,letterSpacing:'.08em',color:'var(--text-muted)',marginBottom:'6px'}}>TRAINING</p>
        <div className="settings-section">
          <div className="settings-row">
            ...
            <select value={`${s.targetWpmMin}-${s.targetWpmMax}`} onChange={e=>{const [min,max]=e.target.value.split('-').map(Number);up({targetWpmMin:min,targetWpmMax:max})}}
      ...
      {/* Notifications */}
      <div className="anim-slide-up anim-d4" style={{marginBottom:'24px'}}>
        <p style={{fontSize:'13px',fontWeight:700,letterSpacing:'.08em',color:'var(--text-muted)',marginBottom:'6px'}}>NOTIFICATIONS</p>
        <div className="settings-section">
          <div className="settings-row">
      ...
      <div className="anim-slide-up anim-d4" style={{marginBottom:'28px'}}>
      ...
      <div className="anim-slide-up anim-d5" style={{background:'rgba(255,48,84,.04)',border:'1px solid rgba(255,48,84,.15)',borderRadius:'20px',padding:'24px'}}>
        ...
        <div className="settings-row" style={{borderBottom:'none',paddingTop:0}}>
          <div><div style={{fontWeight:600}}>Delete All Session Data</div><div className="text-muted" style={{fontSize:'13px'}}>Permanently removes all recordings and stats</div></div>
          <button onClick={handleDelete} style={{background:del?'rgba(255,48,84,.1)':'transparent',border:`1px solid ${del?'var(--hot)':'rgba(255,48,84,.3)'}`,color:'var(--hot)',borderRadius:'100px',padding:'9px 18px',fontSize:'13px',fontWeight:600,cursor:'pointer',fontFamily:'var(--font-body)'}}>
