...
export default function AuthPage() {
  ...
  useEffect(() => {
    if (params.get('error') === 'confirmation_failed') {
      setErr('Confirmation link expired or already used. Please sign up again or request a new link.')
    }
  }, [params])
    ...
    try {
      if (mode === 'signup') {
        ...
        if (!user?.confirmed_at && !user?.email_confirmed_at) {
          ...
          // No confirmation needed — go straight in
  ...
  const resendEmail = async () => {
    ...
    try {
      ...
      if (error) {
        setErr('Could not resend email: ' + error.message)
      } else {
        setCountdown(60)
      }
    } catch {
      ...
      <>
        ...
        <div style={{ minHeight: 'calc(100vh - 73px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
          <div style={{ width: '100%', maxWidth: '440px', textAlign: 'center' }}>
            ...
            <h1 className="font-display anim-slide-up anim-d1" style={{ fontSize: '30px', fontWeight: 900, letterSpacing: '-.03em', marginBottom: '12px' }}>
              Check your email
            </h1>
            <p className="text-muted anim-slide-up anim-d2" style={{ fontSize: '16px', lineHeight: 1.65, marginBottom: '8px' }}>
              We sent a confirmation link to
            </p>
            <p className="anim-slide-up anim-d2" style={{ fontSize: '16px', fontWeight: 600, color: 'var(--accent)', marginBottom: '32px' }}>
              {email}
            </p>
            ...
            {/* Error */}
            {err && (
              <div style={{ background: 'rgba(255,48,84,0.08)', border: '1px solid rgba(255,48,84,0.2)', borderRadius: '12px', padding: '12px 16px', fontSize: '14px', color: 'var(--hot)', lineHeight: 1.5, marginBottom: '16px' }}>
                {err}
              </div>
            )}
            ...
            <div className="anim-slide-up anim-d4">
              ...
              <button
                className={`btn btn-full ${countdown > 0 ? 'btn-outline' : 'btn-outline'}`}
                onClick={resendEmail}
            ...
            <button
              ...
              style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: '13px', cursor: 'pointer', marginTop: '16px', fontFamily: 'var(--font-body)' }}
            >
              ← Back to sign up
            </button>
          </div>
